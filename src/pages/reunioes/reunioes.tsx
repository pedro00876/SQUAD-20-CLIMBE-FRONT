...
export function ReunioesPage() {
  ...
  const participantConflicts = useMemo(
    () =>
      overlappingMeetings.filter((meeting) =>
        meeting.participantIds?.some((participantId) =>
  ...
  const onSubmit = async (data: MeetingFormData) => {
    ...
    try {
      ...
      if (data.participantIds && data.participantIds.length > 0) {
        for (const id of data.participantIds) {
          try {
            ...
            console.error(
              `Erro ao enviar e-mail para participante ${id}:`,
              err
            );
          }
        }
      ...
      setMeetingsForAvailability((currentMeetings) => [
        createdMeeting,
        ...currentMeetings,
      ]);
      ...
      setSubmitError(
        error?.response?.data?.message || 'Não foi possível criar a reunião.'
      );
    } finally {
        ...
        <div className="space-y-6">
                ...
                >
                  ...
                  <div className="mt-6 space-y-3 text-sm text-gray-500">
                    <p>
                      ...
                      {meeting.date || 'Data indisponível'}
                    </p>
                    <p>
                      <span className="font-bold text-climbe-secondary">
                        Horário:
          ...
          <div className="flex items-center justify-between gap-4 border-t border-gray-100 pt-4">
            <p className="text-xs text-gray-400">
              ...
              {Math.max(meetingsResponse?.totalPages ?? 1, 1)}
            </p>
              ...
              <div className="relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[32px] bg-climbe-secondary p-8 text-white shadow-2xl">
                ...
                <div className="mb-6 space-y-2">
                  ...
                  <p className="text-sm text-slate-300">
                    Preencha os dados abaixo para enviar o cadastro para a API.
                  </p>
                </div>
                    ...
                    <div className="max-h-44 space-y-2 overflow-y-auto rounded-xl bg-white p-3 text-slate-900">
                      {isLoadingAvailability ? (
                        ...
                        availableUsers.map((participant) => {
                          ...
                          const participantHasConflict =
                            conflictingParticipantIds.has(participantId);
